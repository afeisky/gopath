...
func Initdb() {
	...
	// 打印执行过程
	verbose := true
	log.Println("Initdb 11111")
	// 遇到错误立即返回
	err := orm.RunSyncdb(name, force, verbose)
	if err != nil {
		fmt.Println(err)
	}
	log.Println("Initdb 222222")
	insertUser()
	insertGroup()
	insertRole()
	insertNodes()
...
func createdb() {
	...
	//sqlstring = fmt.Sprintf("select id,username from user limit 1")
	//r, err := db.Query(sqlstring)
...
func insertUser() {
	...
	u.Username = "test"
	u.Nickname = "bigdata"
	u.Password = Pwdhash("1234")
	u.Email = "chaofei.wu.hz@tcl.com"
	u.Remark = "bigdata test"
	u.Status = 2
	o = orm.NewOrm()
	o.Insert(u)
...
func insertNodes() {
	...
	nodes := [29]Node{
		...
		{Name: "BigData", Title: "Big Data", Remark: "", Level: 1, Pid: 0, Status: 2, Group: g},
			{Name: "BdJson/index", Title: "BdJson", Remark: "", Level: 2, Pid: 25, Status: 2, Group: g},
				{Name: "index", Title: "BdJson", Remark: "", Level: 3, Pid: 26, Status: 2, Group: g},
			{Name: "BdQuery/index", Title: "BdQuery", Remark: "", Level: 2, Pid: 25, Status: 2, Group: g},
				{Name: "index", Title: "BdQuery", Remark: "", Level: 3,Pid: 28, Status: 2, Group: g},
	}
	for _, v := range nodes {
		n := new(Node)
		n.Name = v.Name
...
func insertBdjson() {
	fmt.Println("insert user ...")
	u := new(BdJson)
	u.Filename = "SDT1_32508118005300860108925735632.zip"
	//var a map[string]string
	//a=
	u.Filesave ="{\"data\":{\"imei\":\"232142134213213\"},\"result\":1}"
	o = orm.NewOrm()
	o.Insert(u)
	fmt.Println("insert user end")
}
